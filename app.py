...
def get_repurchase_raw_data(day): 
    ...
    for date in dates:
        try:
            ...
            output = open("repurchase_data/{}.xls".format(end), 'wb')
...
def single_name_information(ticker, df, day):
    ...
    summary = "在{}天内，{}一共回购了{}股，平均回购股价为{} - 共{}元 。截止上一次股东决议会，一共回购了{}股，占流通股百分之{}%".format(day, single_name_df['Stock'].iloc[0], 
    Total_shares, round(Total_paid/Total_shares,2), Total_paid, single_name_df.tail(1)["Number_of_sec_purchased"].iloc[0], single_name_df.tail(1)["Perc_number_of_shares_issue"].iloc[0])
...
def get_stock_industry_summary(ticker, industry_data, raw_repurchase_data):
    ...
    return new_df, num_repurchase, sector_average, Name, sector, 'static/'+str(random) + 'stock_sector_summary.png'
...
def get_entire_industry_summary(df):
    ...
    sector_summary_df = sector_summary_df.sort_values(by ='sum', ascending=False)
...
def restart():
    ...
    for filename in os.listdir("repurchase_data"):
        os.remove("repurchase_data/"+filename)
...
def summary():
    if request.method == 'POST':
        ...
        industry_df, num_repurchase, sector_average, stock_name, sector, photo2 = get_stock_industry_summary(int(ticker), raw_industry, raw_df)
        ...
        return render_template('summary.html', stock_repurchase_data = stock_repurchase_data.to_html(), summary=summary, 
            num_repurchase = num_repurchase, sector_average = sector_average, stock_name = stock_name, sector = sector, photo1 = photo1,
            photo2 = photo2, photo3 = photo3, highest_industry = highest_industry)
